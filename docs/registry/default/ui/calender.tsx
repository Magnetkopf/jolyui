...
const defaultPresets: PresetRange[] = [
  ...
  { label: "Yesterday", getValue: () => ({ from: startOfDay(subDays(new Date(), 1)), to: startOfDay(subDays(new Date(), 1)) }) },
  { label: "Last 7 days", getValue: () => ({ from: startOfDay(subDays(new Date(), 6)), to: startOfDay(new Date()) }) },
  { label: "Last 30 days", getValue: () => ({ from: startOfDay(subDays(new Date(), 29)), to: startOfDay(new Date()) }) },
...
const TimePicker = React.memo(({
  ...
  return (
    <div 
      className="flex items-center justify-center gap-3 py-4 px-2 pointer-events-auto" 
      role="group" 
    ...
    >
      ...
      <div className="flex flex-col items-center gap-1">
        <button
          ...
          disabled={disabled}
          className="p-1.5 rounded-lg hover:bg-accent transition-colors disabled:opacity-50 pointer-events-auto"
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (use24Hour) {
              ...
              updateTime(isPM ? (newHours === 12 ? 12 : newHours + 12) : (newHours === 12 ? 0 : newHours), minutes);
            }
          }}
          disabled={disabled}
          className={cn(
            "font-mono font-bold text-center bg-transparent border-none focus:outline-none focus:ring-2 focus:ring-primary rounded w-12 pointer-events-auto",
            sizes.header
          )}
          aria-label="Hours"
          maxLength={2}
      ...
      {!use24Hour && (
        <button
          ...
          disabled={disabled}
          className="ml-2 px-3 py-2 rounded-lg bg-accent text-sm font-semibold hover:bg-accent/80 transition-colors disabled:opacity-50 pointer-events-auto"
      ...
      {localeStrings.monthsShort.map((month, index) => {
        ...
        return (
          <motion.button
            ...
            disabled={isDisabled}
            className={cn(
              "py-3 px-2 rounded-lg text-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-primary",
              isSelected ? "bg-primary text-primary-foreground shadow-md" : "hover:bg-accent text-foreground",
...
const PresetsPanel = React.memo(({
  ...
  <div className="border-r border-border pr-3 mr-3 space-y-1 min-w-[140px]" role="group" aria-label="Quick date presets">
    <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-2">Quick Select</span>
    {presets.map((preset, index) => (
      <motion.button
        ...
        disabled={disabled}
        className="w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-accent transition-colors focus:outline-none focus:ring-2 focus:ring-primary disabled:opacity-50"
      >
        {preset.label}
      </motion.button>
    ))}
...
const CalendarContent = React.memo(({
  ...
  // Navigation handlers
  const navigate = useCallback((delta: number, type: 'month' | 'year') => {
    setDirection(delta);
    setCurrentMonth(prev => {
      const newDate = type === 'month' 
        ? (delta > 0 ? addMonths(prev, 1) : subMonths(prev, 1))
        : (delta > 0 ? addYears(prev, 1) : subYears(prev, 1));
  ...
  const isDayDisabled = useCallback((day: Date) => {
    ...
    return false;
  }, [disabled, readOnly, minDate, maxDate, disabledDates, disableWeekends, disabledDaysOfWeek, disablePastDates, disableFutureDates]);
  ...
  const handleSelectDate = useCallback((day: Date) => {
    ...
    if (mode === "single") {
      const dateToSet = showTime && value instanceof Date
      ...
      if (!rangeStart) {
        ...
        announce(`Range: ${format(range.from!, 'PP', { locale })} to ${format(range.to!, 'PP', { locale })}`);
      ...
      const newDates = exists
        ...
        : [...currentDates, day];
      onChange?.(newDates);
      announce(`${exists ? 'Deselected' : 'Selected'} ${format(day, 'PP', { locale })}. ${newDates.length} dates selected.`);
    }
  }, [mode, onChange, onClose, rangeStart, showTime, value, closeOnSelect, isDayDisabled, announce, locale]);
  ...
  const getHighlight = useCallback((day: Date) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case "ArrowLeft": newDate = addDays(baseDate, -1); break;
        case "ArrowRight": newDate = addDays(baseDate, 1); break;
        case "ArrowUp": newDate = addDays(baseDate, -7); break;
        case "ArrowDown": newDate = addDays(baseDate, 7); break;
        case "Home": newDate = startOfMonth(baseDate); break;
        case "End": newDate = endOfMonth(baseDate); break;
        case "PageUp": newDate = e.shiftKey ? subYears(baseDate, 1) : subMonths(baseDate, 1); break;
        case "PageDown": newDate = e.shiftKey ? addYears(baseDate, 1) : addMonths(baseDate, 1); break;
        case "Enter":
        case " ":
  ...
  }, [focusedDate, view, currentMonth, value, disabled, readOnly, isDayDisabled, handleSelectDate, onClose, announce, locale]);
  ...
  const handlePresetSelect = useCallback((range: DateRange) => {
    ...
    announce(`Selected: ${range.from && range.to ? `${format(range.from, 'PP')} to ${format(range.to, 'PP')}` : 'preset'}`);
    if (closeOnSelect) onClose?.();
  ...
  const renderDayCell = useCallback((day: Date, monthDate: Date, index: number) => {
    ...
    const dayContent = (
      <motion.button
        ...
        initial={prefersReducedMotion ? false : { opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.1, delay: prefersReducedMotion ? 0 : index * 0.003 }}
        ...
        className={cn(
          sizes.cell,
          "relative rounded-lg font-medium transition-all flex items-center justify-center outline-none",
          !isCurrentMonth && "text-muted-foreground/40",
          isDisabled && "opacity-25 cursor-not-allowed",
          !isSelected && isCurrentMonth && !inRange && "hover:bg-accent text-foreground",
      ...
      >
        ...
        {highlight && (
          <span
            ...
            style={{ backgroundColor: highlight.color || "hsl(var(--primary))" }}
  ...
  }, [sizes.cell, isDaySelected, isDayDisabled, isDayInRange, getHighlight, focusedDate, handleSelectDate, mode, rangeStart, prefersReducedMotion, locale, renderDay]);

  // Render month grid
  const renderMonthGrid = useCallback((monthDate: Date, isSecondary = false) => {
    ...
    return (
      <div className="space-y-1" role="grid" aria-label={format(monthDate, 'MMMM yyyy', { locale })}>
        ...
        {/* Week days header */}
        <div className={cn("grid gap-0.5", showWeekNumbers ? "grid-cols-8" : "grid-cols-7")} role="row">
          {showWeekNumbers && (
            <div className={cn(sizes.cell, "flex items-center justify-center text-xs font-medium text-muted-foreground")} role="columnheader">
              #
            </div>
          )}
          {getWeekDays().map((day, i) => (
            <div
              ...
              className={cn(sizes.cell, "flex items-center justify-center text-xs font-semibold text-muted-foreground")}
            >
              {day}
            </div>
          ))}
            ...
            {days.map((day, index) => {
              ...
              return (
                <React.Fragment key={day.toISOString()}>
                  {showWeekNumber && (
                    <div className={cn(sizes.cell, "flex items-center justify-center text-xs text-muted-foreground")} role="rowheader">
                      {format(day, "w")}
                    </div>
                  )}
  ...
  }, [generateDays, getWeekDays, showWeekNumbers, sizes, direction, prefersReducedMotion, renderDayCell, locale, localeStrings.weekdays, weekStartsOn]);

  const calendarWidth = monthsToShow === 1 ? "w-auto" : monthsToShow === 2 ? "min-w-[580px]" : "min-w-[860px]";
  ...
  return (
    <motion.div
      ...
      transition={{ duration: prefersReducedMotion ? 0 : 0.2 }}
      className={cn(
        "border rounded-xl bg-card shadow-xl shadow-black/10 overflow-hidden pointer-events-auto focus:outline-none focus:ring-2 focus:ring-primary",
        sizes.container,
        calendarWidth,
    ...
    >
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              disabled={disabled}
              className={cn("font-bold px-2 py-1 rounded-lg hover:bg-accent transition-colors", sizes.header)}
        ...
        <AnimatePresence mode="wait">
          {view === "days" && (
            <motion.div key="days" {...(prefersReducedMotion ? {} : fadeScale)} className="flex gap-4">
        ...
        {showTime && mode === "single" && view === "days" && (
          <button
            ...
            disabled={disabled}
            className="w-full mt-3 py-2 rounded-lg flex items-center justify-center gap-2 text-sm font-medium bg-accent/50 hover:bg-accent transition-colors disabled:opacity-50"
          >
            <Clock className="h-4 w-4" />
            {value instanceof Date ? format(value, use24Hour ? "HH:mm" : "hh:mm a") : localeStrings.selectTime}
          </button>
        )}
        ...
        {/* Footer */}
        {(showTodayButton || showClearButton || (mode === "multiple" && value)) && view === "days" && (
          <div className="flex items-center justify-between mt-4 pt-3 border-t border-border/50">
            <div className="flex items-center gap-2">
              {showTodayButton && (
                <button
                  ...
                  onClick={goToToday}
                  disabled={disabled}
                  className="text-xs font-semibold px-3 py-1.5 rounded-md hover:bg-accent transition-colors flex items-center gap-1 disabled:opacity-50"
                >
            ...
            {showClearButton && value && (
              <button
                ...
                onClick={handleClear}
                disabled={disabled}
                className="text-xs font-medium px-3 py-1.5 rounded-md text-muted-foreground hover:bg-accent transition-colors flex items-center gap-1 disabled:opacity-50"
              >
                <X className="h-3 w-3" />
                {localeStrings.clear}
              </button>
...
export function AnimatedCalendar({
  ...
  return (
    <div className="relative">
      ...
      {/* Error message */}
      {error && errorMessage && (
        <p id={errorId} className="mt-1.5 text-xs text-destructive flex items-center gap-1">
